...
export function Simulator() {
  ...
  const visibleChargers = useMemo(() => {
    ...
    let list = search.trim()
      ? data.chargers.filter((c) =>
          matchesContainerSearch(c.identity, null, search, containerOf(c.identity)),
        )
      : data.chargers;
    if (statusFilter) {
      list = list.filter((c) => (statusFilter === "online" ? c.connected : !c.connected));
    }
    return sortByFavorite(list, (c) => c.identity, isFavorite);
  }, [data, search, statusFilter, isFavorite, containerOf]);

  const { simContainerBuckets, simStandalone } = useMemo(() => {
    const buckets = new Map<string, { container: Container; chargers: SimCharger[] }>();
    const loose: SimCharger[] = [];
    for (const c of visibleChargers) {
      const container = containerOf(c.identity);
      if (container) {
        const existing = buckets.get(container.id);
        if (existing) existing.chargers.push(c);
        else buckets.set(container.id, { container, chargers: [c] });
      } else {
        loose.push(c);
      }
    }
    return {
      simContainerBuckets: sortByFavorite(
        [...buckets.values()],
        (b) => b.container.id,
        isFavorite,
      ),
  ...
  }, [visibleChargers, containerOf, isFavorite]);

  const onlineCount = data?.chargers.filter((c) => c.connected).length ?? 0;
  const offlineCount = (data?.chargers.length ?? 0) - onlineCount;
  ...
  function collapseAll() {
    ...
    for (const { container } of simContainerBuckets) keys.add(`container:${container.id}`);
    for (const charger of simStandalone) keys.add(charger.identity);
    setCollapsed(keys);
  }
        ...
        <div className="flex flex-col gap-3">
          ...
          {visibleChargers.length === 0 ? (
            ...
            <div className="space-y-3">
              {simContainerBuckets.map(({ container, chargers }) => (
                <SimContainerCard
                  key={container.id}
                  container={container}
                  chargers={chargers}
              ...
              {simStandalone.map((charger) => (
                <ChargerBench
                  ...
                  carOptions={carOptions}
                  tagOptions={tagOptions}
                  isOpen={!collapsed.has(charger.identity)}
                  onToggle={() => toggleCollapsed(charger.identity)}
...
/** A container of simulated chargers, shown as one collapsible card with
 *  each member's own full ChargerBench nested inside when open -- the same
 *  pattern as Overview's ContainerCard, adapted for this page's own
 *  SimCharger data and controls. */
function SimContainerCard({
  container,
  chargers,
  isOpen,
  onToggle,
  ...
  const onlineCount = chargers.filter((c) => c.connected).length;
        ...
        <div className="space-y-3 border-t border-line p-3">
          {chargers.map((charger) => (
            <ChargerBench
              ...
              carOptions={carOptions}
              tagOptions={tagOptions}
              isOpen={!collapsedChargers.has(charger.identity)}
              onToggle={() => onToggleCharger(charger.identity)}
              isFavorite={isFavorite(charger.identity)}
...
function ChargerBench({
  ...
  return (
    <Panel className="overflow-hidden">
      <div className="flex flex-wrap items-center gap-3 bg-panel-high/30 px-4 py-3">
        <button
          type="button"
          onClick={onToggle}
          className="flex items-center gap-1.5 text-ink-faint hover:text-ink"
          aria-label={isOpen ? `Collapse ${charger.identity}` : `Expand ${charger.identity}`}
        >
        ...
        {onRemoveFromContainer && (
          <button
            ...
            className="rounded-md p-1 text-ink-faint hover:text-signal-fault"
            aria-label={`Remove ${charger.identity} from its container`}
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <Button onClick={() => setBlocking(true)}>
            <Ban size={13} /> Blocking
          </Button>
      ...
      {blocking && (
        <BlockActionsModal identity={charger.identity} onClose={() => setBlocking(false)} />
      )}
...
/** Human-readable description for each blockable command, so the switch
 *  list reads as "what this does" rather than a bare protocol name. */
const ACTION_LABELS: Record<BlockableAction, { title: string; hint: string }> = {
  RemoteStartTransaction: {
    title: "Remote start",
    hint: "The dashboard's Start button will be refused outright.",
  },
  RemoteStopTransaction: {
    title: "Remote stop",
    hint: "End will be refused; a session already open stays open.",
  },
  SetChargingProfile: {
    title: "Set charging profile",
    hint: "Stop (pause) will be refused; nothing gets held at 0 W.",
  },
  ClearChargingProfile: {
    title: "Clear charging profile",
    hint: "Resume will be refused; a real hold cannot be lifted.",
  },
  ChangeConfiguration: {
    title: "Change configuration",
    hint: "Configuration edits sent from the dashboard will be refused.",
  },
  Reset: {
    title: "Reset",
    hint: "Soft and hard reset commands will be refused.",
  },
  UnlockConnector: {
    title: "Unlock connector",
    hint: "Remote unlock will report failure rather than releasing the cable.",
  },
  TriggerMessage: {
    title: "Trigger message",
    hint: "Requests to re-send a status, heartbeat, or meter reading are refused.",
  },
};

/** Reject specific OCPP commands outright, for this one charger.
 *
 *  Every switch here sends a real, genuine rejection reply over the wire
 *  when it is on -- the same honest "no" the handler already sends for its
 *  own real reasons elsewhere, just switched on deliberately. This is for
 *  testing how the CSMS behaves when a charger simply will not honour a
 *  given command, which real hardware in the field can absolutely do.
 */
function BlockActionsModal({
  identity,
  onClose,
}: {
  identity: string;
  ...
  const { data } = useBlockedActions(identity);
  const toggle = useToggleBlockedAction();

  return (
    <Modal title={`Blocking — ${identity}`} onClose={onClose} footer={<Button onClick={onClose}>Done</Button>}>
      <p className="mb-3 text-xs text-ink-faint">
        Switch an action on to make this charger refuse it outright, with a
        real rejection reply, every time the CSMS sends it.
      </p>
      <div className="space-y-2">
        {BLOCKABLE_ACTIONS.map((action) => {
          const isBlocked = Boolean(data?.actions?.[action]);
          const { title, hint } = ACTION_LABELS[action];
          return (
            <div
              key={action}
            ...
            >
              <button
                ...
                onClick={() =>
                  toggle.mutateAsync({ identity, action, blocked: !isBlocked })
                }
                className={cx(
                  "relative h-6 w-11 shrink-0 rounded-full border disabled:opacity-60",
                  isBlocked
