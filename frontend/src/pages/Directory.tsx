...
function VehicleEditor({
  ...
  const [profile, setProfile] = useState(vehicle?.charge_profile ?? "generic");
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    ...
    >
      ...
      <Field
        label="Charge curve"
        hint="Which real DC fast-charging shape this car follows"
      >
        <Select
          value={profile}
          onChange={(v) => setProfile(v as typeof profile)}
          options={[
            { value: "generic", label: "Generic — flat to 80%, tapers after" },
            { value: "renault", label: "Renault Megane E-Tech — peaks early, steady taper" },
            { value: "tesla", label: "Tesla — very high peak, sharp late drop" },
            { value: "hyundai_kia", label: "Hyundai/Kia E-GMP — flat plateau, cliff at 80%" },
            { value: "vw_id", label: "VW ID.4 — gentle, wide taper" },
            { value: "nissan_leaf", label: "Nissan Leaf — lower peak, early gradual taper" },
          ]}
        />
      </Field>
    </Modal>
