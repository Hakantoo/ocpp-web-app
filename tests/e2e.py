...
async def run() -> None:
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        run = await client.post(
            f"{CSMS}/api/stress-tests",
            json={
                "name": "e2e stress check",
                "steps": [
                    {"kind": "create", "count": 2, "connectors": 1},
                    {"kind": "plug_in"},
                    {"kind": "present_card"},
                    {"kind": "charge"},
                    # Deliberately out of order: remote-stopping a connector
                    # that already has power offered through the C switch,
                    # not through a genuine RemoteStartTransaction, is a real
                    # thing to prove the runner honestly reports rather than
                    # silently succeeding or crashing the whole run.
                    {"kind": "remote_stop"},
                    {"kind": "delete"},
                ],
            },
        )
        check("stress test starts", run.status_code == 201, run.text[:200])
        run_id = run.json()["id"]

        finished = await poll(
            lambda: _get(client, f"{CSMS}/api/stress-tests/{run_id}"),
            lambda r: r["status"] in ("done", "failed"),
        )
        check("stress test reaches a real end state", finished["status"] == "done",
              finished["status"])
        step_kinds = [s["kind"] for s in finished["steps"]]
        check("every step actually ran", step_kinds ==
              ["create", "plug_in", "present_card", "charge", "remote_stop", "delete"])

        create_result = finished["steps"][0]
        check("create step reports real numbers, not just a count",
              "connectors each" in create_result["detail"], create_result["detail"])

        plug_result = finished["steps"][1]
        check("plug_in step succeeded on every connector",
              "plugged in 2 connectors" in plug_result["detail"], plug_result["detail"])

        delete_result = finished["steps"][-1]
        check("delete step reports what it removed",
              "deleted 2 chargers" in delete_result["detail"], delete_result["detail"])

        after_delete = await _get(client, f"{CSMS}/api/charge-points")
        check("delete step genuinely removed the chargers it created",
              not any(c["identity"].startswith(run_id) for c in after_delete))

        # -- 14b. saved test definitions -------------------------------------------
        saved = await client.post(
            f"{CSMS}/api/test-definitions",
            json={"name": "e2e saved test", "steps": [{"kind": "wait", "seconds": 0.1}]},
        )
        check("saved test creates", saved.status_code == 201, saved.text[:200])
        def_id = saved.json()["id"]

        listed = await _get(client, f"{CSMS}/api/test-definitions")
        check("saved test appears in the list",
              any(d["id"] == def_id for d in listed))

        edited = await client.patch(
            f"{CSMS}/api/test-definitions/{def_id}",
            json={"steps": [{"kind": "wait", "seconds": 0.2}, {"kind": "wait", "seconds": 0.3}]},
        )
        check("saved test's steps can be edited, not just its name",
              edited.status_code == 200)
        after_edit = await _get(client, f"{CSMS}/api/test-definitions")
        edited_def = next(d for d in after_edit if d["id"] == def_id)
        check("the edit genuinely took", len(edited_def["steps"]) == 2)

        run_saved = await client.post(f"{CSMS}/api/test-definitions/{def_id}/run")
        check("a saved test can be run", run_saved.status_code == 201, run_saved.text[:200])

        deleted = await client.delete(f"{CSMS}/api/test-definitions/{def_id}")
        check("saved test deletes", deleted.status_code == 200)
        after_delete_def = await _get(client, f"{CSMS}/api/test-definitions")
        check("deleted saved test is genuinely gone",
              not any(d["id"] == def_id for d in after_delete_def))
