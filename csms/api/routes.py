...
from ..repository import test_definitions as test_definitions_repo
...
ChargeProfile = Literal[
    "generic", "renault", "tesla", "hyundai_kia", "vw_id", "nissan_leaf"
]
...
class VehicleBody(BaseModel):
    ...
    charge_profile: ChargeProfile = "generic"
...
# Stress testing
#
# A run is a short, ordered sequence of steps against real, genuinely
# simulated hardware -- not a synthetic shortcut. It lives entirely in
# memory on the CSMS process (see domain/stress_test.py); the only
# persistent trace it leaves is the real charge_points/id_tags/vehicles rows
# its own steps create, all under one run-specific identity prefix, which is
# exactly what a Delete step uses to find and remove everything a run
# touched and nothing else.
# ---------------------------------------------------------------------------


class StressStepBody(BaseModel):
    kind: Literal[
    ...
    connectors: int = Field(default=2, ge=1, le=8)
    seconds: float = Field(default=1.0, ge=0, le=3600)
    delete_target: Literal["all", "created_here"] = "created_here"


class StressRunBody(BaseModel):
    name: str = Field(min_length=1, max_length=80)
    steps: list[StressStepBody] = Field(min_length=1, max_length=50)


def _stress(request: Request):
    return request.app.state.stress_tests


@router.post("/stress-tests", status_code=201)
async def start_stress_test(request: Request, body: StressRunBody) -> dict[str, Any]:
    from ..domain.stress_test import Step
    ...
    steps = [
        Step(
            ...
            seconds=s.seconds,
            delete_target=s.delete_target,
        )
        for s in body.steps
    ]
    run = await _stress(request).start(body.name, steps)
    return _run_summary(run)


@router.get("/stress-tests")
async def list_stress_tests(request: Request) -> list[dict[str, Any]]:
    from ..domain.stress_test import list_runs

    return [_run_summary(r) for r in list_runs()]


@router.get("/stress-tests/{run_id}")
async def get_stress_test(request: Request, run_id: str) -> dict[str, Any]:
    from ..domain.stress_test import get_run

    run = get_run(run_id)
    if run is None:
        raise HTTPException(404, f"No such test run {run_id!r}")
    return _run_summary(run)


@router.post("/stress-tests/{run_id}/cancel")
async def cancel_stress_test(request: Request, run_id: str) -> dict[str, Any]:
    ok = _stress(request).cancel(run_id)
    if not ok:
...
def _run_summary(run: Any) -> dict[str, Any]:
    return {
        ...
        "name": run.name,
        "status": run.status,
        "created_count": len(run.created_identities),
        "steps": [
            {
                "kind": r.kind,
                "ok": r.ok,
...
#
# A definition is a reusable, named template -- distinct from an actual run,
# which stays transient and in-memory. This is what makes "create a test
# once, run it whenever" possible: the definition survives a restart and is
# shared across whoever is using this CSMS, the same way a real charger or
# a saved card is.
# ---------------------------------------------------------------------------


class TestDefinitionBody(BaseModel):
    name: str = Field(min_length=1, max_length=80)
    steps: list[StressStepBody] = Field(min_length=1, max_length=50)


class TestDefinitionRename(BaseModel):
    name: str | None = Field(default=None, min_length=1, max_length=80)
    steps: list[StressStepBody] | None = Field(default=None, min_length=1, max_length=50)


@router.post("/test-definitions", status_code=201)
async def create_test_definition(
    request: Request, body: TestDefinitionBody
) -> dict[str, Any]:
    async with _db(request).transaction() as conn:
        definition_id = await test_definitions_repo.create(
            conn, name=body.name, steps=[s.model_dump() for s in body.steps]
        )
    return {"id": definition_id, "name": body.name, "steps": [s.model_dump() for s in body.steps]}


@router.get("/test-definitions")
async def list_test_definitions(request: Request) -> list[dict[str, Any]]:
    async with _db(request).acquire() as conn:
        return await test_definitions_repo.list_all(conn)


@router.patch("/test-definitions/{definition_id}")
async def rename_test_definition(
    request: Request, definition_id: int, body: TestDefinitionRename
) -> dict[str, Any]:
    async with _db(request).transaction() as conn:
        existing = await test_definitions_repo.get(conn, definition_id)
        if existing is None:
            raise HTTPException(404, f"No such saved test {definition_id}")
        if body.name is not None:
            await test_definitions_repo.rename(conn, definition_id, body.name)
        if body.steps is not None:
            await test_definitions_repo.update_steps(
                conn, definition_id, [s.model_dump() for s in body.steps]
            )
    return {"ok": True}


@router.delete("/test-definitions/{definition_id}")
async def delete_test_definition(request: Request, definition_id: int) -> dict[str, Any]:
    async with _db(request).transaction() as conn:
        await test_definitions_repo.delete(conn, definition_id)
    return {"ok": True}


@router.post("/test-definitions/{definition_id}/run", status_code=201)
async def run_test_definition(request: Request, definition_id: int) -> dict[str, Any]:
    """Start a real run from a saved definition -- the exact same
    orchestration a freshly-built test gets, just sourced from the saved
    step list instead of a one-off request body."""
    async with _db(request).acquire() as conn:
        definition = await test_definitions_repo.get(conn, definition_id)
    if definition is None:
        raise HTTPException(404, f"No such saved test {definition_id}")

    from ..domain.stress_test import Step

    steps = [Step(**s) for s in definition["steps"]]
    run = await _stress(request).start(definition["name"], steps)
    return _run_summary(run)