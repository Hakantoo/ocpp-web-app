...
run with the `sqlite3` CLI. Thirteen tables and two views.
...
|---|---|
| `/` | every connector with its meter tape and Start / Stop / End controls |
| `/chargers` | hardware inventory and online state |
...
| `/directory` | RFID cards and vehicles, both editable |
| `/logs` | every frame, filterable and expandable to the full payload |
| `/simulator` | plug, unplug, present a card, inject a fault |
