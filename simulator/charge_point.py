...
class SimulatedChargePoint(BaseChargePoint):
    def __init__(
        ...
        *,
        ...
        # gets the genuine "no" its own retry/error-handling logic already
        # has to cope with, not a timeout.
    ...
    async def on_remote_start(
        ...
        if "RemoteStartTransaction" in self.blocked_actions:
    ...
    async def on_remote_stop(
        ...
        if "RemoteStopTransaction" in self.blocked_actions:
    ...
    async def on_set_charging_profile(
        ...
        if "SetChargingProfile" in self.blocked_actions:
            return call_result.SetChargingProfile(status=ChargingProfileStatus.rejected)
    ...
    async def on_clear_charging_profile(
        ...
        if "ClearChargingProfile" in self.blocked_actions:
    ...
    async def on_change_configuration(
        ...
        if "ChangeConfiguration" in self.blocked_actions:
    ...
    async def on_reset(self, type: str) -> call_result.Reset:
        if "Reset" in self.blocked_actions:
    ...
    async def on_unlock(self, connector_id: int) -> call_result.UnlockConnector:
        if "UnlockConnector" in self.blocked_actions:
    ...
    async def on_trigger_message(
        ...
        if "TriggerMessage" in self.blocked_actions:
