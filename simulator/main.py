...
# The exact set of inbound CSMS commands the simulator can genuinely refuse
# with a real, honest OCPP rejection status -- kept to actions that have a
# meaningful "no" in the spec (a plain read like GetConfiguration has none,
...
@app.post("/block-action")
async def block_action(body: BlockActionRequest) -> dict:
    """Reject a specific inbound OCPP command outright, for this charger.

    A real rejection reply goes back over the wire -- the same genuine "no"
    every handler already sends for its own real reasons (a fault, an
    unknown profile), just switched on deliberately rather than triggered
    by a real condition. This is for testing how the CSMS behaves when a
    charger simply will not honour a given command, which real hardware in
    the field absolutely can do (an unsupported feature, a firmware bug, a
    deliberate policy).
    """
    if body.action not in BLOCKABLE_ACTIONS:
        raise HTTPException(
            400,
            f"{body.action!r} cannot be blocked -- only {', '.join(BLOCKABLE_ACTIONS)}",
        )
    cp = _cp(body.identity)
    if body.blocked:
        cp.blocked_actions.add(body.action)
    else:
        cp.blocked_actions.discard(body.action)
    return {"ok": True, "action": body.action, "blocked": body.blocked}


@app.get("/block-action")
async def list_blocked_actions(identity: str) -> dict:
    """Every blockable action and whether it is currently blocked, for one
    charger -- what the dashboard's per-charger menu actually renders."""
    cp = _cp(identity)
    return {
        "identity": identity,
        "actions": {
            action: action in cp.blocked_actions for action in BLOCKABLE_ACTIONS
        },
    }
